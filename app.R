...
ui <- navbarPage(theme = shinytheme("slate"), 
                 ...
                 tabPanel('Risk Map',
                          sidebarLayout(
                            ...
                            mainPanel(width = 9,
                              leafletOutput("map",  width= 1100, height=750)
                            ))),
...
server <- function(input, output) {
  ...
  w_tot <- eventReactive(input$render,{sum(input$w1,input$w2, input$w3, input$w4)}, 
                         ignoreNULL = FALSE)
  ...
  pal <- eventReactive(input$render, {if (input$col == "Yellow-Red"){return (pal_1)}
                                      else if(input$col == "Green-Yellow-Red"){return (pal_2)}
                                      else if(input$col == "Blue-Yellow-Red"){return (pal_3)}
                                      }, ignoreNULL = FALSE)
  
  op <- eventReactive(input$render, {input$opacity}, ignoreNULL = FALSE)
                                      
  
  # Reactive expression to select historic fire polygons for years 
  # the user selects
  #filteredHistData <- eventrReactive(input$render,
  ...
  output$map <- renderLeaflet({
    leaflet(options = leafletOptions(minZoom = 9)) %>%
      ...
      addLegend(position = "bottomleft", pal = pal(), values = values(asset_raster),
                title = "Risk",
                opacity = 1.0) %>% 
